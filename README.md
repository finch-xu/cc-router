...
app 启动后会在后台静默检查新版本，发现后侧边栏「关于」入口会出现红点提示。进入「关于」页可以看到新版本号、release notes，点「立即更新」开始下载（带 ed25519 签名校验）。下载完成后**不会立即重启**——cc-router 是常驻代理，重启会中断 Claude Code 当前会话；点「立即重启」由你决定时机。

平台支持：

- **macOS / Windows / Linux AppImage**：原地自动更新
- **Linux `.deb`**：仅检测到新版本提示，需手动下载安装包重装（plugin-updater 不支持 deb 原地升级）
